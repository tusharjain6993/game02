...
import { shootinginfo } from '../data/data'
...
function Shooting(){
    ...
    return(
        <>        
        <div className="shooting-backcover">
            {
                shootinginfo.map(function(d){
                    return(
                        <div className="shooting-name-div">
                        <Row className='m-3'>

                            <Col lg={6}><img src={d.image} style={{ width: '100%', height: '100%' }} />
                                </Col>
                            <Col lg={6}>
                            <div className="shooting-name">
                                        <h3 style={{ margin: '0', padding: '0' }}>{d.name}</h3>
                                    </div>
                                    <div className="shooting-os">
                                        <h3>{d.OperatingSystem}</h3>
                                    </div>
                                    <div className="shooting-cpu">
                                        <h3>{d.CPU}</h3>
                                    </div>
                                    <div className="shooting-ram">
                                        <h3>{d.RAM}</h3>
                                    </div>
                                    <div className="shooting-setupsize">
                                        <h3>{d.SetupSize}</h3>
                                    </div>
                                    <div className="shooting-hardisk">
                                        <h3>{d.HardDiskSpace}</h3>
                                    </div>
                                    <Button className="shooting-button" onClick={() => nav('/detail', { state: { product: d } })}>
                                        PURCHASE
                                    </Button>
                            </Col>
                        </Row></div>
