...
import { racinginfo } from '../data/data'
...
function Sports(){
    ...
    return(
        <>        
        <div className="racing-backcover">
            {
                racinginfo.map(function(d){
                    return(
                        <div className="racing-name-div">
                        <Row className='m-3'>

                            <Col lg={6}><img src={d.image} style={{ width: '100%', height: '100%' }} />
                                </Col>
                            <Col lg={6}>
                            <div className="racing-name">
                                        <h3 style={{ margin: '0', padding: '0' }}>{d.name}</h3>
                                    </div>
                                    <div className="racing-os">
                                        <h3>{d.OperatingSystem}</h3>
                                    </div>
                                    <div className="racing-cpu">
                                        <h3>{d.CPU}</h3>
                                    </div>
                                    <div className="racing-ram">
                                        <h3>{d.RAM}</h3>
                                    </div>
                                    <div className="racing-setupsize">
                                        <h3>{d.SetupSize}</h3>
                                    </div>
                                    <div className="racing-hardisk">
                                        <h3>{d.HardDiskSpace}</h3>
                                    </div>
                                    <Button className="racing-button" onClick={() => nav('/detail', { state: { product: d } })}>
                                        PURCHASE
                                    </Button>
                            </Col>
                        </Row></div>
